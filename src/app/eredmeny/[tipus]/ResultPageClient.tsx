...
export default function ResultPageClient() {
  ...
  const primaryPct = searchParams.get('primaryPct');
  const secondaryPct = searchParams.get('secondaryPct');

  const primary = typeData[primaryType];
  const secondary = typeData[secondaryType];

  // Generate percentages - use URL params if available, otherwise calculate mock percentages
  const { percentages, scores } = useMemo(() => {
    if (primaryPct && secondaryPct) {
      // Use URL-provided percentages
      const pPct = parseInt(primaryPct);
      const sPct = parseInt(secondaryPct);
      const remaining = 100 - pPct - sPct;
      const otherTypes = Object.keys(typeData).filter(
        (t) => t !== primaryType && t !== secondaryType
      ) as DesignerType[];

      const pcts: Record<DesignerType, number> = {} as Record<DesignerType, number>;
      const scores: Record<DesignerType, number> = {} as Record<DesignerType, number>;

      pcts[primaryType] = pPct;
      pcts[secondaryType] = sPct;
      scores[primaryType] = Math.round(pPct / 10);
      scores[secondaryType] = Math.round(sPct / 10);

      const perOther = Math.floor(remaining / otherTypes.length);
      otherTypes.forEach((type, i) => {
        pcts[type] = i === 0 ? remaining - perOther * (otherTypes.length - 1) : perOther;
        scores[type] = Math.round(pcts[type] / 10);
      });

      return { percentages: pcts, scores };
    }

    // Fallback: generate mock percentages
  ...
  }, [primaryType, secondaryType, primaryPct, secondaryPct]);
  ...
  const shareUrl =
    typeof window !== 'undefined'
      ...
      : `${process.env.NEXT_PUBLIC_BASE_URL}/eredmeny/${primaryType}?alt=${secondaryType}&primaryPct=${percentages[primaryType]}&secondaryPct=${percentages[secondaryType]}`;
